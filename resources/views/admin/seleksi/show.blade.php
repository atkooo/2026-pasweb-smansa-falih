...
@if($isLocked)
    <div class="alert alert-warning border-0 shadow-sm mb-4" style="border-radius: 0.75rem; background: #fffbebf5; border-left: 5px solid #f59e0b !important;">
            ...
            <div class="card-body">
                <form action="{{ route('seleksi.store', $pendaftaran->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_seleksi" class="form-control" required {{ $isLocked ? 'disabled' : '' }} style="border-radius: 8px;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" class="form-control" {{ $isLocked ? 'disabled' : '' }} style="border-radius: 8px;" rows="3" placeholder="Catatan khusus..."></textarea>
                    </div>
                    @if($isLocked)
                        <button type="button" disabled class="btn btn-secondary btn-block font-weight-bold" style="border-radius: 8px;"><i class="fas fa-lock mr-2"></i> Form Terkunci</button>
                    @else
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 align-middle text-right">
                                    @if($isLocked)
                                        <button disabled class="btn btn-sm btn-light text-muted rounded-circle" title="Nilai Terkunci">
                ...
                @if($isLocked)
                    <div class="row align-items-center">
                        <div class="col-md-8 mb-3 mb-md-0">
                            <p class="text-muted mb-2">Status kelulusan akhir peserta telah ditetapkan dan dikunci. Nilai dan status tidak dapat diubah kembali.</p>
                            <div class="d-flex align-items-center">
                                <label class="mr-3 font-weight-bold mb-0">Status Akhir:</label>
                                @if($pendaftaran->status_kelulusan === 'LOLOS')
                                    <span class="badge badge-success px-3 py-2" style="font-size: 0.9rem;"><i class="fas fa-check-circle mr-1"></i> LOLOS</span>
                                @else
                                    <span class="badge badge-danger px-3 py-2" style="font-size: 0.9rem;"><i class="fas fa-times-circle mr-1"></i> TIDAK LOLOS</span>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="input-group">
                                <select disabled class="form-control font-weight-bold bg-light" style="border-radius: 8px 0 0 8px;">
                                    <option selected>{{ $pendaftaran->status_kelulusan }}</option>
                                </select>
                                <div class="input-group-append">
                                    <button class="btn btn-secondary font-weight-bold" disabled type="button" style="border-radius: 0 8px 8px 0;"><i class="fas fa-lock mr-1"></i> Terkunci</button>
                                </div>
                            </div>
                        </div>
                    </div>
