    ...
    <div class="card shadow-sm border-0" style="border-radius: 0.75rem;">
        <div class="card-header bg-white border-bottom pt-4 pb-3 d-flex justify-content-between align-items-center">
            <h6 class="font-weight-bold text-dark mb-0" style="text-transform: uppercase; letter-spacing: 0.5px;">DAFTAR
                PENGUMUMAN</h6>
            <a href="{{ route('pengumuman.create') }}" class="btn btn-primary btn-sm rounded-pill px-3"><i
                    ...
                    <thead class="bg-light">
                        <tr>
                            <th class="border-top-0 border-bottom-0 text-muted px-4"
                                style="font-size: 0.85rem; font-weight: 600;">#</th>
                            <th class="border-top-0 border-bottom-0 text-muted"
                                style="font-size: 0.85rem; font-weight: 600;">JUDUL PENGUMUMAN</th>
                            <th class="border-top-0 border-bottom-0 text-muted"
                                style="font-size: 0.85rem; font-weight: 600;">TANGGAL PUBLIKASI</th>
                        ...
                        @forelse($pengumumans as $index => $p)
                            <tr>
                                ...
                                <td class="py-3 font-weight-bold">
                                    ...
                                    @if($p->lampiran)
                                        <a href="{{ Storage::url($p->lampiran) }}" target="_blank" class="badge badge-secondary"><i
                                                class="fas fa-paperclip"></i> Ada Lampiran</a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('pengumuman.edit', $p->id) }}"
                                        class="btn btn-sm btn-outline-primary rounded-pill px-3 font-weight-bold"><i
                                    ...
                                    <form action="{{ route('pengumuman.destroy', $p->id) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="btn btn-sm btn-outline-danger rounded-pill px-3 font-weight-bold"><i
