...
<div class="mb-4 mt-2">
    ...
    <p class="text-muted" style="font-size: 0.95rem;">Kelola status penerimaan calon anggota baru dan atur tahun periode aktif.</p>
</div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center mb-3">
                    <div class="rounded-circle d-flex align-items-center justify-content-center mr-3"
                        ...
                        <i class="fas {{ $statusPendaftaran === 'buka' ? 'fa-door-open text-success' : 'fa-door-closed text-danger' }}" style="font-size: 1.1rem;"></i>
                    </div>
                    <div>
                ...
                <div class="d-flex align-items-center mb-4 py-3 px-4 rounded-lg"
                    ...
                    <div>
                        @if($statusPendaftaran === 'buka')
                            <p class="mb-0 text-success font-weight-bold" style="font-size: 0.9rem;">Pendaftaran Sedang Berjalan</p>
                ...
                {{-- Toggle Button --}}
                <form action="{{ route('admin.pengaturan-sistem.toggle') }}" method="POST"
                    id="form-toggle-status"
                    onsubmit="return confirmToggle('{{ $statusPendaftaran }}')">
                    @csrf
                    <button type="submit" id="btn-toggle-status"
                        class="btn btn-block font-weight-bold py-2"
                        style="border-radius: 0.6rem; font-size: 0.95rem;
        ...
        <div class="card border-0 shadow-sm h-100" style="border-radius: 1rem;">
            <div class="card-body p-4">
                ...
                <div class="text-center py-3 mb-4 rounded-lg"
                    ...
                    <p class="text-muted mb-1" style="font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.5px;">Periode Aktif Saat Ini</p>
                    <h1 class="font-weight-bold mb-0" style="font-size: 3rem; color: #4f46e5; letter-spacing: -2px;">{{ $tahunAktif }}</h1>
                    <small class="text-muted">Angkatan {{ $tahunAktif }}</small>
                </div>

                {{-- Form Ganti Tahun --}}
                <form action="{{ route('admin.pengaturan-sistem.update') }}" method="POST"
                    onsubmit="return confirmGantiTahun()">
                    @csrf
                    <div class="form-group mb-3">
                        <label class="font-weight-600 text-muted small text-uppercase mb-2">Ganti Tahun Periode Aktif</label>
                        <div class="input-group">
                            <input type="number" name="tahun_aktif" id="input-tahun-aktif"
                                class="form-control @error('tahun_aktif') is-invalid @enderror"
                                value="{{ old('tahun_aktif', $tahunAktif) }}"
                ...
                <div class="alert mb-0 py-2 px-3" style="background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.3); border-radius: 0.5rem;">
                    <small class="text-warning font-weight-bold">
                        <i class="fas fa-exclamation-triangle mr-1"></i>
                        Mengganti tahun aktif tidak akan menghapus data tahun sebelumnya. Data lama tetap tersimpan sebagai arsip.
                    </small>
                </div>
            </div>
        </div>
    ...
    <div class="card-header bg-white border-bottom pt-4 pb-3" style="border-radius: 1rem 1rem 0 0;">
        <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h6 class="font-weight-bold text-dark mb-0" style="text-transform: uppercase; letter-spacing: 0.5px;">Rekap Arsip Per Angkatan</h6>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0" style="vertical-align: middle;">
                <thead class="bg-light">
                    <tr>
                        <th class="border-top-0 border-bottom-0 text-muted px-4" style="font-size: 0.82rem; font-weight: 700;">TAHUN / ANGKATAN</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-center" style="font-size: 0.82rem; font-weight: 700;">TOTAL PENDAFTAR</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-center" style="font-size: 0.82rem; font-weight: 700;">DISETUJUI</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-center" style="font-size: 0.82rem; font-weight: 700;">DINYATAKAN LULUS</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-center" style="font-size: 0.82rem; font-weight: 700;">TIDAK LULUS</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-center" style="font-size: 0.82rem; font-weight: 700;">STATUS</th>
                        <th class="border-top-0 border-bottom-0 text-muted text-right px-4" style="font-size: 0.82rem; font-weight: 700;">AKSI</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td class="px-4 py-3">
                            <div class="d-flex align-items-center">
                                <div class="rounded-circle mr-3 d-flex align-items-center justify-content-center"
                                    ...
                                    <i class="fas fa-calendar {{ $arsip->tahun_periode == $tahunAktif ? 'text-primary' : 'text-secondary' }}" style="font-size: 0.8rem;"></i>
                                </div>
                                <div>
                                    <span class="font-weight-bold text-dark" style="font-size: 1rem;">{{ $arsip->tahun_periode }}</span>
                                    <br>
                        ...
                        <td class="py-3 text-center">
                            <span class="font-weight-bold" style="font-size: 1.1rem; color: #1f2937;">{{ $arsip->total_pendaftar }}</span>
                        ...
                        <td class="py-3 text-center">
                            @if($arsip->tahun_periode == $tahunAktif)
                                <span class="badge badge-pill px-3 py-2 font-weight-bold"
                                    ...
                                    <i class="fas {{ $statusPendaftaran === 'buka' ? 'fa-circle text-success' : 'fa-circle text-danger' }}" style="font-size: 0.5rem; vertical-align: middle;"></i>
...
<script>
    function confirmToggle(currentStatus) {
        const action = currentStatus === 'buka' ? 'MENUTUP' : 'MEMBUKA';
        const detail = currentStatus === 'buka'
            ? 'Calon anggota tidak akan bisa mengakses formulir pendaftaran.'
            : 'Calon anggota dapat mengakses dan mengisi formulir pendaftaran.';
        return confirm(`⚠️ Konfirmasi ${action} Pendaftaran\n\n${detail}\n\nLanjutkan?`);
    }

    function confirmGantiTahun() {
        const tahun = document.getElementById('input-tahun-aktif').value;
        return confirm(`⚠️ Konfirmasi Ganti Tahun Aktif\n\nAnda akan mengubah tahun periode aktif menjadi "${tahun}".\nData dari tahun sebelumnya akan otomatis menjadi arsip.\n\nLanjutkan?`);
    }
</script>
@endsection
