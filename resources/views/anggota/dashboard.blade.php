                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8 d-flex align-items-center mb-4 mb-md-0">
                            <div class="position-relative mr-4 mr-md-5">
                                @if(auth()->user()->foto)
                                    <img src="{{ asset('storage/' . auth()->user()->foto) }}" alt="{{ auth()->user()->nama_lengkap }}" class="rounded-circle shadow" style="width: 80px; height: 80px; object-fit: cover; border: 3px solid white;">
                                @else
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0" style="border-radius: 1rem;">
                <div class="card-header bg-white border-0 pt-4 pb-0 d-flex justify-content-between align-items-center">
                    <h4 class="m-0 font-weight-bold text-dark"><i class="fas fa-file-alt text-success mr-2"></i> Formulir Pendaftaran Saya</h4>
                    <a href="{{ route('profil-pengguna.index') }}" class="btn btn-outline-primary btn-sm rounded-pill font-weight-bold px-3">
                ...
                <div class="card-body p-4">
                    <div class="row">
                        <div class="col-md-3 col-sm-6 mb-3">
                            ...
                            <span class="text-dark font-weight-bold" style="font-size: 1.05rem;">{{ $formulir->nama_panggilan }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Asal Sekolah</small>
                            <span class="text-dark font-weight-bold" style="font-size: 1.05rem;">{{ $formulir->asal_sekolah }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Tinggi / Berat Badan</small>
                            <span class="text-dark font-weight-bold" style="font-size: 1.05rem;">{{ $formulir->tinggi_badan }} cm / {{ $formulir->berat_badan }} kg</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Pengalaman PBB (SMP/MTs)</small>
                            <span class="badge badge-info px-3 py-2 rounded-pill font-weight-bold" style="font-size: 0.85rem;">{{ $formulir->opsi_pilihan === 'YA' ? 'Pernah (YA)' : 'Belum (TIDAK)' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="row mt-2 pt-3 border-top">
                        <div class="col-md-3 col-sm-6 mb-3">
                            ...
                            <span class="text-dark">{{ $formulir->tempat_lahir }}, {{ \Carbon\Carbon::parse($formulir->tanggal_lahir)->format('d M Y') }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Kontak (WhatsApp)</small>
                            <span class="text-dark"><i class="fab fa-whatsapp text-success mr-1"></i> {{ $formulir->no_hp }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Periode Angkatan</small>
                            <span class="text-dark"><i class="fas fa-calendar-check text-primary mr-1"></i> Tahun {{ $formulir->tahun_periode }}</span>
                        </div>
                        <div class="col-md-3 col-sm-6 mb-3">
                            <small class="text-muted d-block font-weight-bold">Status Pendaftaran</small>
                            <span class="badge badge-success px-3 py-1 rounded-pill"><i class="fas fa-check-circle mr-1"></i> Diterima / Anggota</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-top">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="font-weight-bold text-dark m-0"><i class="fas fa-award text-warning mr-2"></i> Ringkasan Nilai & Status Kelulusan Seleksi</h6>
                            @php
                                $totalAkhir = 0;
                            ...
                            @endphp
                            <span class="badge badge-primary px-3 py-2 rounded-pill font-weight-bold" style="font-size: 0.9rem;">
                            ...
                            @foreach($kriterias as $k)
                                ...
                                <div class="col-md-3 col-sm-6 mb-2">
                                    <div class="p-3 border rounded bg-light">
                                        <small class="text-muted d-block font-weight-bold text-truncate">{{ $k->nama }} ({{ $k->bobot }}%)</small>
                                        <div class="d-flex justify-content-between align-items-center mt-1">
                                            <span class="font-weight-bold text-dark" style="font-size: 1.1rem;">{{ number_format($nilaiK, 1) }}</span>
                                            @if($hasilK)
                                                @if($isLulusK)
                        ...
                        <div class="row align-items-center mt-2">
                            <div class="col-4 text-center">
                                <div class="position-relative d-inline-block">
                                    @if(auth()->user()->foto)
                                        <img src="{{ asset('storage/' . auth()->user()->foto) }}" alt="{{ auth()->user()->nama_lengkap }}" class="rounded-circle shadow" style="width: 80px; height: 80px; object-fit: cover; border: 3px solid #fbbf24; margin: 0 auto; display: block;">
                                    @else
                            ...
                            <div class="col-8 pl-1">
                                ...
                                <p class="mb-1 text-warning font-weight-bold" style="font-size: 0.78rem;">
                                    NO. KTA: KTA-{{ auth()->user()->formulirPendaftaran->tahun_periode ?? date('Y') }}/00{{ auth()->user()->id }}
                                </p>
                                <div style="font-size: 0.73rem; opacity: 0.95;" class="text-light">
                                    <div class="mb-1"><i class="fas fa-id-badge text-warning mr-1" style="width: 14px;"></i> NISN: <strong>{{ auth()->user()->nisn ?? '-' }}</strong></div>
                                    <div class="mb-1 text-truncate"><i class="fas fa-school text-warning mr-1" style="width: 14px;"></i> {{ auth()->user()->formulirPendaftaran->asal_sekolah ?? 'SMA Negeri 1 Pontianak' }}</div>
                                    <div><i class="fas fa-calendar-check text-warning mr-1" style="width: 14px;"></i> Angkatan: Tahun {{ auth()->user()->formulirPendaftaran->tahun_periode ?? date('Y') }}</div>
                                </div>
                            </div>
                        </div>
